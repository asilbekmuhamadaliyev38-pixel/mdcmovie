...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        # Admin bo'lsa, admin rejimine o'tkazamiz (state tozalanadi)
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ==================== REJIM ALMASHTIRISH TUGMALARI ====================
    # Bu tugmalar HAMMA VAQT ishlaydi (state'dan qat'i nazar)

    if is_admin(user_id) and text == "👤 Foydalanuvchi rejimiga o'tish":
        admin_states[user_id] = "user_mode"
        new_movies_temp.pop(user_id, None)
        save_data_local()
        await send_welcome_message(update.message, user_id=user_id, is_admin_user=True)
        return

    if is_admin(user_id) and text == "👑 Admin rejimiga o'tish":
        admin_states[user_id] = None
        save_data_local()
    ...
    # Admin "user_mode"da bo'lsa yoki oddiy foydalanuvchi bo'lsa

    if not is_admin(user_id) or is_in_user_mode(user_id):
        # Obuna tekshiruvi
        if not await is_joined(context.bot, user_id):
            reply_markup = await get_subscription_keyboard(context.bot)
    ...
    # ==================== ADMIN REJIMI ====================
    # Faqat haqiqiy admin va user_mode'da EMAS bo'lganda
    ...
    if state == "add_movie_data":
        ...
        if not video_id.isdigit():
            await update.message.reply_text(
                "❌ 4-qator (Post ID) faqat raqam bo'lishi kerak!\n\nQaytadan yuboring:",
                reply_markup=get_cancel_keyboard()
            )
            return
        ...
        admin_states[user_id] = "add_movie_poster_photo"
        save_data_local()
        await update.message.reply_text(
            f"✅ Qabul qilindi!\n\n"
            f"🎬 {name.upper()} | Kod: {code}\n\n"
            f"🖼️ Endi poster rasmini yuboring:\n"
            f"(Rasm yubormasdan o'tkazib yuborish uchun /skip yozing)",
            reply_markup=get_cancel_keyboard()
        )
        return

    elif state == "add_movie_poster_photo":
        # Rasm o'rniga matn kelsa
        if text == "/skip":
            # Postersiz saqlash
            movie_data = new_movies_temp.get(user_id, {})
            preview = (
                f"🎬 Nomi: {movie_data.get('name', '').upper()}\n"
                f"📝 Ma'lumot: {movie_data.get('desc', '')}\n"
                f"🔑 Kod: {movie_data.get('code', '')}\n"
                f"🖼️ Poster: Yo'q\n"
                f"📥 Video ID: {movie_data.get('video_id', '')}\n\n"
                f"Tasdiqlaysizmi?"
            )
    ...
    if state is None:
        if text == "➕ Kino qo'shish":
            ...
            await update.message.reply_text(
                ...
                "avengers\n"
                "12345\n\n"
                "⬇️ Keyin poster rasmini yuborasiz",
                reply_markup=get_cancel_keyboard()
            )
            return
