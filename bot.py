...
# Bekor qilish tugmasi — har doim bosh panelga qaytadi
def get_cancel_keyboard():
    return ReplyKeyboardMarkup([["❌ Bekor qilish"]], resize_keyboard=True)

# Bosh panelga qaytish — amal bajarilganda yoki bekor qilinganda
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BEKOR QILISH — har qanday holatda bosh panelga qaytadi
    ...
    if is_admin(user_id) and state:
        if state == "add_movie_name":
            ...
            await update.message.reply_text(
                "📥 4-Qadam: Kanaldagi Post ID raqamini yuboring:",
                reply_markup=get_cancel_keyboard()
            )
            return
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BEKOR QILISH — inline tugmadan bosh panelga qaytish
